...
class Image
{
     ...
     */
    public static function imageResize($picname,$path,$maxWidth,$maxHeight,$pre="s_",$pro="m_",$pra="b_"){
        $path = rtrim($path,"/")."/";
        //1获取被缩放的图片信息
        ...
        switch($info[2]){
		 ...
		 switch($info[2]){
            case 1: //gif格式
                imagegif($dstim,$path.$pro.$picname);
                break;
            case 2: //jpeg格式
                imagejpeg($dstim,$path.$pro.$picname);
                break;
            case 3: //png格式
                imagepng($dstim,$path.$pro.$picname);
                break;
        }
		switch($info[2]){
            case 1: //gif格式
                imagegif($dstim,$path.$pra.$picname);
                break;
            case 2: //jpeg格式
                imagejpeg($dstim,$path.$pra.$picname);
                break;
            case 3: //png格式
                imagepng($dstim,$path.$pra.$picname);
                break;
        }
